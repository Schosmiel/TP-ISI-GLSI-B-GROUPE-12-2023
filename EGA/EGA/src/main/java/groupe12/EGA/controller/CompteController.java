...
import groupe12.EGA.model.*;
import groupe12.EGA.payload.PostCompte;
import groupe12.EGA.repo.*;
...
public class CompteController {
    ...
    @DeleteMapping(value ="/supprimer/{numCpt}")
    ...
    public ResponseEntity<?> enregistrerCompte(@RequestBody PostCompte postcompte) {
        try {
            Client proprietaire = clientRepo.findById(postcompte.getProprietaire())
                    .orElseThrow(() -> new EntityNotFoundException("Client non trouvé avec l'ID " + postcompte.getProprietaire()));
